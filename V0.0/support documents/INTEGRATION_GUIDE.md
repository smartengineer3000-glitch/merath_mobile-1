...
describe("Enhanced Calculation Engine", () => {
  ...
  it("should calculate blood relatives distribution", () => {
    ...
    expect(result.success).toBe(true);
    expect(
      result.specialCases?.some((sc) => sc.type === "blood_relatives"),
    ).toBe(true);
  });
});
```
...
```typescript
const oldResult = new InheritanceCalculationEngine(
  madhab,
  estate,
  heirs,
).calculate();
const newResult = new EnhancedInheritanceCalculationEngine(
  madhab,
  estate,
  heirs,
).calculate();
...
console.log(
  "Match:",
  Math.abs(oldResult.shares[0].amount - newResult.shares[0].amount) < 0.01,
);
```
