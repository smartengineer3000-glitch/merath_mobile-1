  ...
  static async restoreFromBackup(): Promise<void> {
    try {
      ...
      if (backupData.data.settings) {
        await AsyncStorage.setItem('@merath_settings_v2', JSON.stringify(backupData.data.settings));
      }
      if (backupData.data.favorites) {
        await AsyncStorage.setItem('@merath_favorites', JSON.stringify(backupData.data.favorites));
      }
