...
export class CSVExporter {
  ...
  static exportToCSV(
    ...
    const { includeSummary = true, includeDetails = true } = options;

    const rows: string[][] = [];
    const netEstate =
      result.netEstate || result.shares.reduce((sum, s) => sum + s.amount, 0);
    ...
    if (includeSummary) {
      ...
      rows.push([
        "إجمالي التركة:",
        result.netEstate?.toFixed(2) || netEstate.toFixed(2),
      ]);
