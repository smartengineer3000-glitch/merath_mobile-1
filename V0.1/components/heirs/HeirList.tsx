...
export function HeirList({ selectedHeirs }: HeirListProps) {
  ...
  const entries = Object.entries(selectedHeirs).filter(([_, count]) => count > 0);
  ...
  return (
    <View style={styles.container}>
      {entries.map(([key, count]) => {
        ...
        return (
          <View key={key} style={[styles.row, { borderColor: theme.colors.neutral.light200 }]}>
            <View style={styles.left}>
              <Avatar icon={heir.icon} color={heir.color} size={28} />
              <Text style={[styles.name, { color: theme.colors.neutral.dark200, fontFamily: theme.fontFamily.english }]}>
                {key.replace(/_/g, " ")}
              </Text>
            </View>
