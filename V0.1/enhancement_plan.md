...
- Add `format:check` script to `package.json` using Prettier check mode.
- Add an Expo doctor check to CI or documented release commands.
- Ensure `npm run check`, `npm run lint`, `npm test`, and format check run from the `V0.1` directory.
- Document any environment-only blocker in this tracking file after each run.
