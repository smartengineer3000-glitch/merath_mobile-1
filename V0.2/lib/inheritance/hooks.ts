...
export function useCalculator() {
  ...
  const calculateWithMethod = useCallback(
    async (
      ...
      const calculationEstateData = estateOverride ?? estateData;

      try {
        // Validate estate data
        if (calculationEstateData.total <= 0) {
        ...
        const calculationPromise = new Promise<CalculationResult>((resolve) => {
          debouncedCalculate(madhab, heirs, calculationEstateData, resolve);
        });
