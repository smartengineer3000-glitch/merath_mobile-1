...
export class AuditLog {
  ...
  private async initializeDatabase(): Promise<void> {
    try {
      ...
      if (db && typeof db.auditLogs !== "undefined") {
        ...
        console.log(
          `[AuditLog] Loaded ${this.entries.length} entries from IndexedDB`,
        );
      } else {
   ...
   */
  private async filterUsingIndexedDB(
    filterCriteria: AuditLogFilter,
  ): Promise<AuditLogEntry[]> {
    try {
      ...
      if (filterCriteria.madhab) {
        query = query.filter(
          (entry) => entry.madhab === filterCriteria.madhab,
        ) as any;
      }

      if (filterCriteria.operation) {
        query = query.filter(
          (entry) => entry.operation === filterCriteria.operation,
        ) as any;
      }

      if (filterCriteria.successOnly !== undefined) {
        query = query.filter(
          (entry) => entry.metadata.success === filterCriteria.successOnly,
        ) as any;
      }
      ...
      if (filterCriteria.dateFrom) {
        ...
        query = query.filter(
          (entry) => new Date(entry.timestamp) >= fromDate,
        ) as any;
      }
      ...
      if (filterCriteria.dateTo) {
        ...
        query = query.filter(
          (entry) => new Date(entry.timestamp) <= toDate,
        ) as any;
      }
      ...
      console.error(
        "[AuditLog] IndexedDB filter failed, falling back to memory:",
        error,
      );
      return this.filterInMemory(filterCriteria);
    }
  ...
  async exportAsJSON(): Promise<string> {
    ...
    if (this.dbReady && this.entries.length < (await db.auditLogs.count())) {
      entriesToExport = (await db.auditLogs.toArray()).map(
        this.convertFromDBEntry,
      );
    }
