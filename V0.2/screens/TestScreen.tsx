...
export default function TestScreen() {
  ...
  const runBasicTest = async () => {
    const result = await calculateWithMethod("shafii", {});
    setTestResults([{ name: "Basic Test", result, passed: !!result?.success }]);
  };

  const runCustomTest = async () => {
    try {
      const input = JSON.parse(customInput) as {
        estate?: {
          total: number;
          funeral: number;
          debts: number;
      ...
      setTestResults((prev) => [
        ...prev,
        { name: "Custom Test", result, passed: !!result?.success },
      ]);
    } catch (error) {
    ...
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Test inheritance calculations and edge cases
        </Text>
      </View>
      ...
      <Card style={styles.card}>
        ...
        <Text style={styles.cardSubtitle}>
          Run a standard inheritance calculation test
        </Text>
      ...
      <Card style={styles.card}>
        ...
        <Text style={styles.cardSubtitle}>
          Enter custom test data (JSON format)
        </Text>
        <TextInput
          style={styles.input}
          value={customInput}
          onChangeText={setCustomInput}
          placeholder='{"estate": {"total": 100000, "funeral": 0, "debts": 0, "will": 0}, "heirs": {"son": 1}}'
          multiline
        />
